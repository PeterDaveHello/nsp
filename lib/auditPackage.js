...
function auditPackage(pkgPath, cb) {
        ...
        function vulnerable(module, cb) {
            validateModule(module.name, module.version, function (err, result) {
                ...
                if (!Array.isArray(result)) {
                  return cb(new TypeError('Unexpected API response format.'), null);
                }
                ...
                dependencyOf = dependencyOf.map(function (d) { return d.name + '@' + d.version; });
    ...
    function mapPackage(pkginfo, fn, cb) {
        ...
        var dependencies = (pkginfo.dependencies && Object.keys(pkginfo.dependencies)) || [];
        async.mapLimit(dependencies, MAX_ASYNC_THROTTLING, function (dependencyName, cb2) {
            var dependencyVersion = pkginfo.dependencies[dependencyName];

            // Ignore git dependencies
            if (isGitDependency(dependencyVersion)) {
                console.warn('Warning: Git dependency ['  + dependencyName + '@' + dependencyVersion + '] ignored.');
                return cb2(null);
            }

            if (isLocalDependency(dependencyVersion)) {
                console.warn('Warning: Local dependency ['  + dependencyName + '@' + dependencyVersion + '] ignored.');
                return cb2(null);
            }

            registry.get(dependencyName, dependencyVersion, function (err, dependencyPkginfo) {
                if (err) { return cb2(err); }
                if (!dependencyPkginfo || !dependencyPkginfo.versions) {
                    return cb2(null);
                }

                var ver = semver.maxSatisfying(Object.keys(dependencyPkginfo.versions), dependencyVersion);

                if (!dependencyPkginfo.versions[ver]) {
                  console.warn('Error processing: ['  + dependencyPkginfo.name + '@' + dependencyPkginfo.version + '] ignored.');
                  return cb2(null);
                }
                ...
                if (processed[dependencyPkginfo.name + '@' + dependencyPkginfo.version]) {
                    return cb2(null);
                }

                mapPackage(dependencyPkginfo, fn, function (err, rootResult, mappedDependencies) {
                    if (err) { return cb(err); }
                    var res = [];
                    if (rootResult) { res.push(rootResult); }
                    cb2(null, res.concat(mappedDependencies));
                });
            });
        }, function (err, mappedDependencies) {
            mappedDependencies = Array.prototype.concat.apply([], mappedDependencies);
            // remove undefined / null from ignored packages
            mappedDependencies = mappedDependencies.filter(function (dependency) {
                return dependency;
            });
            if (err) { return cb(err); }
